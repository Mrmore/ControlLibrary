...
namespace ControlLibrary.GifSynthesis
{
    ...
    public class LZWEncoder
    {
        ...
        void Add(byte c, IRandomAccessStream outs)
        {
            accum[a_count++] = c;
            if (a_count >= 254)
                Flush(outs);
        ...
        void ClearTable(IRandomAccessStream outs)
        {
            ResetCodeTable(hsize);
            free_ent = ClearCode + 2;
            clear_flg = true;
        ...
        void Compress(int init_bits, IRandomAccessStream outs)
        {
            int fcode;
            int i /* = 0 */;
            int c;
        ...
        void Output(int code, IRandomAccessStream outs)
        {
            cur_accum &= masks[cur_bits];
