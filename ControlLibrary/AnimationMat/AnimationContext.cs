...
namespace ControlLibrary.AnimationMat
{
    /// <summary>
    ...
    {
        internal AnimationContext(UIElement target, Storyboard storyboard, params object[] args)
            : this(target)
        {
        ...
        {
            this.SingleProperty(new PropertyPath(propertyPath), args);
        }

        internal void SingleProperty(DependencyProperty propertyPath, params double[] args)
        {
            this.SingleProperty(new PropertyPath(propertyPath), args);
        }

        internal void Discrete(DependencyProperty propertyPath, params object[] args)
        {
            // convert arguments to double
            double[] doubleArgs = new double[args.Length];
            for (int i = 0; i < args.Length; i++)
            {
                doubleArgs[i] = Convert.ToDouble(args[i], CultureInfo.InvariantCulture);
            }

            this.SingleProperty(new PropertyPath(propertyPath), doubleArgs);
        }
