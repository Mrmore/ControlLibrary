    ...
    public sealed class ImageGif : Control, IDisposable
    {
        ...
        private void StopChanged()
        {
            if (this.Stop)
            {
                ...
                if (this.imageGifGrid != null && this.imageList != null
                    && this.bitmapImageList != null && this.bitFrame != null)
                {
                    this.imageGifGrid.Children.Clear();
                    //做优化
                    this.imageList.Clear();
        ...
        private async void Start(string uri)
        {
            ...
            if (GifCacheDictionaryHelper.Instance.SelectUriToBytes(this.Source) == null)
            {
                ...
                {
                    var ss = System.Uri.IsWellFormedUriString(this.Source, UriKind.RelativeOrAbsolute);
        ...
        private async void dispatcherTimer_Tick(object sender, object e)
        {
            if (bitFrame != null && bitFrame.Count > 0)
            {
                if (counter == bitFrame.Count)
                {
                    //正常
                    counter = 0;
                }

                var frame = bitFrame.ElementAt(counter);
                var imageGif = imageList.ElementAt(counter);
                ...
                if (frame != null)
                {
                    ...
                    {
                        BitmapImage bmp = bitmapImageList.ElementAt(counter);
                        ...
                        else
                        {
                            if (bitFrame.Count > 0)
                            {
                                if (!bitFrame.ElementAt(counter - 1).Disposal)
                                {
                                    imageList[counter - 1].Visibility = Windows.UI.Xaml.Visibility.Collapsed;
                                }
                            }
                        }
                        try
                    ...
                    dispatcherTimer.Interval = frame.Delay;
                }
                counter++;
            }
        }
